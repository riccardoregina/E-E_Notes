...
\[I(X;Y) \geq I(X;Y|Z)\]
Intuitivamente, questa disuguaglianza ci dice che, se \(X \to Y \to Z\), l'informazione che \(Y\) fornisce su \(X\) non può essere aumentata conoscendo \(Z\). Ciò avviene perché la conoscenza di $Z$ riduce l'incertezza su $Y$ ma non su $X$ e quindi riduce anche la loro mutua informazione. 
...
Allora, per l'indipendenza, si ha \(I(X;Y) = 0\), ma 
...
Uno stato $x \in \mathcal{X}$ è periodico, con periodo $n$, quando tutti i cammini che partono da $x$ e vi ritornano hanno lunghezza multiplo di $n$. Inoltre, $n$ è il massimo periodo osservato, ovvero il $\gcd$ della definizione. Se $\gcd=1$, allora lo stato si dice aperiodico. Una catena si dice aperiodica quando tutti i suoi stati sono aperiodici. Una catena periodica ha un comportamento, appunto, periodico e quindi, asintoticamente, non si stabilizza ad una distribuzione stazionaria.
...
\begin{note}{}
  Per una qualsiasi distribuzione $\mu$, rappresentata mediante vettore riga, possono essere usate indistintamente le notazioni $\mu_i$ e $\mu(i)$ per indicare la probabilità dell'$i$-esimo simbolo di $\mathcal{X}$.
\end{note}

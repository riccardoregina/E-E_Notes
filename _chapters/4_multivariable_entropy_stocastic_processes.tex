...
\begin{proposition}[label=prop:general-chain-rule]{}
...
\begin{definition}{Catena di Markov (3 variabili)}
  Date tre variabili aleatorie \(X, Y, Z\), si dice che esse formano una \keyword{catena di Markov}, nell'ordine denotato come \(X \to Y \to Z\) se vale che
  \[\forall x \in X, y \in Y, z \in Z: p(x,y,z) = p(x)p(y|x)p(z|y)\]
\end{definition}
Tre variabili $X,Y,Z$ possono formare una catena di Markov anche in diverso ordine. Tuttavia, non è generalmente vero che se formano una catena in un ordine, allora esse ne formano una anche in ordine diverso.

\begin{observation}{}
  Nel caso di $X,Y,Z$ variabili qualsiasi, si osserva
  \[p(x,y,z)=p(x)p(y|x)p(z|x,y)\]
  Se $X \to Y \to Z$, si verifica anche $p(z|x,y)=p(z|y)$ e ciò avviene se e soltanto se $X$ e $Z$ sono, dato $Y$, indipendenti. In tal caso, si verifica $p(x,z|y)=p(x|y)p(z|y)$.
  In altre parole, in una catena di Markov $X \to Y \to Z$, il valore di una variabile dipende esclusivamente da quello della variabile che la precede immediatamente.
\end{observation}
...
\begin{proof}
  ...
  \begin{description}
    \item[(Caso $z \neq f(y)$)]
      In questo caso, si verifica \[\sum_{x\in\mathcal{X}}p(x,z|y)=p(z|y)=0\]
      Tale somma è composta da termini non negativi, che, quindi, sono tutti nulli. Quindi, $\forall x\in\mathcal{X}:p(x,z|y)=0=p(x|y)p(z|y)$ poiché due quantità nulle sono ovviamente uguali.
    \item[(Caso $z = f(y)$)]
      Si ha \(\sum_{k\in\mathcal{Z}}p(x,k|y)=p(x,z|y)\) poiché l'unico termine non nullo della somma sarà quello per $k=z$. 
      Inoltre, si ha \(\sum_{k\in\mathcal{Z}}p(x,k|y)=p(x|y)\). Allora, 
      \[\begin{cases}
        \sum_{k\in\mathcal{Z}}p(x,k|y)=p(x,z|y) \\
        \sum_{k\in\mathcal{Z}}p(x,k|y)=p(x|y)
      \end{cases} \implies p(x,z|y)=p(x|y)=p(x|y)\cdot 1=p(x|y)p(z|y)\]
  \end{description}
  In ogni caso, \(p(x,z|y)=p(x|y)p(z|y)\), quindi si ha $X \to Y \to Z$. 
\end{proof}
...
\begin{proof}
  ...
  Allora
  \[I(X;Y) = I(X;Z) + I(X;Y|Z) \]
  Essendo \(I(X;Y|Z) \geq 0\), si ha la tesi.
...
\[I(X;Y) \geq I(X;Y|Z)\]
Intuitivamente, questa disuguaglianza ci dice che, se \(X \to Y \to Z\), l'informazione che \(Y\) fornisce su \(X\) non può essere aumentata conoscendo \(Z\). Ciò avviene perché la conoscenza di $Z$ riduce l'incertezza su $Y$ ma non su $X$ e questo fa sì che $I(X;Y)$ sia generalmente maggiore di $I(X;Y|Z)$. 

In generale, però, se non è vero \(X \to Y \to Z\), tale disuguaglianza può non sussistere.
Ad esempio, siano \(X, Y\) i.i.d\footnote{Indipendenti e Identicamente Distribuite}, uniformi su \(\{0,1\}\), e sia \(Z = X + Y\), con \(\mathcal{Z} = \set{0,1,2}\).
Allora, per l'indipendenza tra $X$ e $Y$, si ha \(I(X;Y) = 0\), ma 
\[I(X;Y|Z) = H(X|Z) - \bcancel{H(X|Y,Z)} = \frac{1}{2} > I(X;Y)\]
poiché la conoscenza di $Z$ dà informazione sia su $X$ che su $Y$.

\begin{definition}{Processi stocastici}
  Un \keyword{processo stocastico} \(\underline{X} = {(X_n)}_{n\geq 1}\) è una successione di variabili aleatorie, dette variabili membro, su \(\mathcal{X}\).
  ...
  In particolare, per \(n=1\) si ha che le variabili membro \(X_i\) sono identicamente distribuite.

  Inoltre, se le variabili aleatorie sono anche indipendenti, si ha un processo \keyword{i.i.d.}
\end{definition}

Un processo stazionario può essere utilizzato come modello per una sorgente che emette simboli nel tempo secondo una certa distribuzione.
Come caso particolare, se le variabili membro sono indipendenti, allora il processo può modellare una sorgente a memoria zero.
...
L'analogia con le sorgenti a memoria zero per i processi stazionari indipendenti potrebbe portare a pensare che le catene di Markov rappresentino solo sorgenti con memoria \(1\), ma, in realtà, aggregando più variabili aleatorie insieme, è possibile modellare sorgenti con memoria maggiore. Ad esempio, si potrebbero prendere variabli aleatorie su $\mathcal{X}^{n}$ e quindi ottenere una catena di Markov che ha la capacità di memorizzare $n$ simboli di $\mathcal{X}$.
...
\begin{definition}{Catene tempo invarianti}
  ...
  ovvero se la probabilità di emissione di un simbolo, dato il simbolo precedente, non dipende dal tempo.
\end{definition}

Una catena di Markov tempo invariante è completamente caratterizzata dalla distribuzione iniziale \(\mu^{(1)}=\Prob[X_1 = x]\) e da una matrice \(P\), detta \keyword{matrice di transizione}, tale che \(P_{i,j} = (\Prob[X_2 = j | X_1 = i])\).\footnote{La definizione della matrice di transizione assume che \(\mathcal{X} = \set{1, \ldots, \abs{\mathcal{X}}}\). Se ci si trovasse con un alfabeto che non rispetta questa ipotesi, si potrebbe sempre costruire una corrispondenza biunivoca tra gli elementi di \(\mathcal{X}\) e un insieme di interi consecutivi che parte da $1$.} 
Una riga di $P$ corrisponde ad una legge di probabilità condizionata. Ad esempio, $P_{i}$ è il vettore riga che rappresenta la distribuzione di probabilità di emissione dei simboli dato che l'ultimo simbolo emesso è $i$. 
Per questo motivo, una riga di $P$ somma sempre ad $1$: $P$ è una matrice stocastica\footnote{Una matrice è detta stocastica per righe (colonne) quando le sue righe (colonne) sono costituite da elementi non negativi che hanno somma unitaria. Quando si parla di matrice stocastica, si intende implicitamente che essa è stocastica per righe. Inoltre, una matrice è detta doppiamente stocastica quando è stocastica sia per righe che per colonne.}.

Come detto, una catena invariante nel tempo è completamente caratterizzata dalla distribuzione iniziale $\mu^{(1)}$ e dalla matrice di transizione $P$. Infatti,
\[\mu^{(2)}_j = \Prob[X_{2}=j] = \sum_{i \in \mathcal{X}}\Prob[X_2 = j | X_1 = i]\Prob[X_1 = i] = \sum_{i \in \mathcal{X}} P_{i,j}\mu^{(1)}_i = \mu^{(1)} \cdot P^{T}_{j}\]
ovvero $\mu^{(2)}_{j}$ viene calcolato con il prodotto tra $\mu^{(1)}$, vettore riga, e la $j$-esima colonna di $P$. 
Di conseguenza, la distribuzione $\mu^{(2)}$ viene calcolata come
\[\mu^{(2)}=\mu^{(1)}\cdot P\]

A questo punto, siamo interessati a studiare il comportamento asintotico di una catena di Markov, ovvero come essa si comporta ad un tempo infinitamente distante dal suo inizio.

\begin{definition}{Distribuzione stazionaria}
Rispetto a una matrice di transizione \(P\), una distribuzione di probabilità \(\mu\) su \(\mathcal{X}\) si dice \keyword{stazionaria} se \(\mu P = \mu\), ovvero se \(\mu\) è un autovettore sinistro di \(P\) associato all'autovalore \(1\).
\end{definition}

Se un processo \(\underline{X}\) è invariante nel tempo e si osserva $\mu_1=\mu$, allora esso ha $\mu$ come distribuzione in ogni istante di tempo. In questo caso, \(\underline{X}\) si dice \keyword{invariante}.

\begin{definition}{Catene irriducibili}
  Una catena \(\underline{X}\) si dice \keyword{irriducibile} se
  \[\forall i,j \in \mathcal{X}: \exists n \geq 1 \st \Prob[X_{n+1} = j|X_1 = i] > 0\]
\end{definition}

Una catena è detta irriducibile se, in ogni istante, è possibile raggiungere ogni stato da ciascuno stato in un numero finito di passi. In altre parole, una catena è irriducibile se nella rappresentazine a grafo\footnote{È possibile rappresentare una catena di Markov mediante un grafo i cui nodi sono i simboli di $\mathcal{X}$ ed un arco da $x$ a $y$ ed etichettato con $p(y|x)$ rappresenta la probabilità di passare dallo stato $x$ allo stato $y$.} della catena non sono presenti \qi{isole}, ovvero regioni da cui, una volta entrati, non è possibile tornare indietro, rendendo impossibile il raggiungimento degli stati al di fuori di tale regione. 
Nello studio del comportamento asintotico delle catene, si considerano quelle irriducibili per minimalità: asintoticamente, una catena riducibile può essere ridotta, appunto, ad una sua \qi{isola}. Si noti che, se sono presenti più \qi{isole}, allora la distribuzione asintotica non è unica. 

\begin{definition}{Catene aperiodiche}
  Una catena \(\underline{X}\) si dice \keyword{aperiodica} se
  \[\forall x \in \mathcal{X} \st \gcd \set{n\in\N}[\Prob[X_{n+1} = x|X_1=x]>0] = 1\]
\end{definition}

Uno stato $x \in \mathcal{X}$ è periodico, con periodo $n$, quando tutti i cammini che partono da $x$ e vi ritornano hanno lunghezza multiplo di $n$ che è il massimo periodo osservato, ovvero il $\gcd$ della definizione. Se $\gcd=1$, allora lo stato si dice aperiodico. Una catena si dice aperiodica quando tutti i suoi stati sono aperiodici. Una catena periodica ha un comportamento, appunto, periodico e quindi, asintoticamente, non si stabilizza ad una distribuzione stazionaria.

\begin{theorem}{}
  Sia $\underline{X}$ una catena di Markov irriducibile, aperiodica e invariante nel tempo. Allora esiste un'unica distribuzione stazionaria $\mu$ t.c. 
  \[\lim_{n\to\infty} \mu^{(n)} = \mu\]
  che non dipende dalla distribuzione iniziale.
\end{theorem}

\begin{note}{}
  Per una qualsiasi distribuzione $\mu$, rappresentata mediante vettore riga, possono essere usate indistintamente le notazioni $\mu_i$ e $\mu(i)$ per indicare la probabilità di emissione dell'$i$-esimo simbolo di $\mathcal{X}$.
\end{note}
...
\begin{example}{}
  ...
  \begin{equation*}
    ...
    \end{pmatrix}
  \end{equation*}
  Se \(\mu_n = (\Prob[X_n = 1], \Prob[X_n = 2])\), allora \(\mu_{n+1} = \mu_n P\).
...
Il tasso entropico è un tentativo di generalizzare ai processi il concetto di entropia congiunta. Esso è un tentativo poiché non è detto che il limite esista.

Se le variabili aleatorie \(X_i\) sono i.i.d., allora, dalla regola di catena (\Cref{prop:general-chain-rule}), si ha
  \[H(X_1, X_2, \ldots, X_n) = H(X_1) + \sum_{i=2}^n H(X_i|X_{i-1}, \ldots, X_1) = \sum_{i=1}^n H(X_i) = n H(X_1)\]

Dunque, in questo caso, si osserva \(H(\underline{X}) = H(X_1)\).\footnote{Essendo le variabili membro i.i.d si ha che \(H(X_1) = H(X_i), \forall i\), cioè potrebbe essere scelta l'entropia di una qualsiasi delle variabili membro. Di solito, per comodità, si sceglie $X_1$.}

Si può mostrare però, che, già togliendo la sola ipotesi di indipendenza, il limite può non esistere.
...
\begin{proof}
  \(\forall n \geq 1\), poiché che il condizionamento non aumenta l'entropia, si ha che
  \[H(X_{n+1}|X_n, \ldots, X_1)\leq H(X_{n+1}|X_n, \ldots, X_2)\]
  Poiché \(\underline{X}\) è stazionario, si ha che
  ...
  Inoltre, dalla regola di catena, si ha 
  \[\frac{1}{n} H(X_1, \ldots, X_n) = \frac{1}{n}\left(H(X_1) +\sum_{i=2}^n H(X_i|X_{i-1}, \ldots, X_1)\right)\]
  cioè \(\frac{1}{n} H(X_1, \ldots, X_n)\) è la media dei primi \(n\) termini della successione \(a_n\).

  Per il teorema della media di Cesàro\footnote{Teorema di Cesàro: sia $(a_{n})_{n}$ una successione convergente e $(m_{n})_{n}$ la successione della media aritmetica dei primi $n$ valori di $(a_{n})_{n}$ tale per cui $m_{n}=\frac{a_{1}+\dots +a_{n}}{n}$. Allora$$\lim_{ n \to \infty }{m_{n}}=\lim_{ n \to \infty }{a_{n}}$$}, si ricava \(H(\underline{X}) = H'(\underline{X})\).
\end{proof}
...
Essendo il tasso di entropia una quantità definita al limite, tutte le proprietà che abbiamo visto valgono anche per catene di Markov non stazionarie ma la cui distribuzione converge ad un'unica distribuzione stazionaria, ovvero, come visto, le catene invarianti nel tempo, aperiodiche e irriducibili.

\begin{example}{}
  Per la catena di Markov dell'esempio precedente, per la formula appena vista, si ha che
  \[H(\underline{X}) = -\mu(1)\left((1-\alpha)\log(1-\alpha) + \alpha \log \alpha\right) - \mu(2)\left(\beta \log \beta + (1-\beta) \log (1-\beta)\right)\]

...
Se un processo \(\underline{X}\) è invariante nel tempo e si osserva $\mu^{(1)}=\mu$, allora esso ha $\mu$ come distribuzione in ogni istante di tempo. In questo caso, \(\underline{X}\) si dice \keyword{invariante}.
...
Uno stato $x \in \mathcal{X}$ è periodico, con periodo $n$, quando tutti i cammini che partono da $x$ e vi ritornano hanno lunghezza multiplo di $n$, con $n$ massimo, ovvero il $\gcd$ dei periodi, come nella definizione. Se $\gcd=1$, allora lo stato si dice aperiodico. Una catena si dice aperiodica quando tutti i suoi stati sono aperiodici. Invece, una catena periodica ha un comportamento, appunto, periodico e quindi, asintoticamente, non si stabilizza ad una distribuzione stazionaria.
